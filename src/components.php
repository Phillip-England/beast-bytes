...
class Component {
    ...
    public static function navMenu() {
        ?>
            <nav class='fixed top-0 left-0 w-3/5 h-screen z-40 border-r hidden'>
    ...
    public static function navItem(string $text, string $href) {
        ...
        ?>
            <li class='flex border rounded-sm bg-white-500'>
                <a href=<?= $href ?> class='p-4 w-full'><?= $text ?></a>
            </li>
        <?php
    }
    ...
    public static function navOverlay() {
        ?>
            <div class='fixed hidden top-0 left-0 w-full h-full bg-black-500 cursor-pointer bg-opacity-50 z-30'></div>
        <?php
    }
